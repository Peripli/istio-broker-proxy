...
func TestConsumerPostUnbind(t *testing.T) {
	...
	g.Expect(configStore.DeletedIstioConfigs).Should(ContainElement(Equal("destination-rule:egressgateway-svc-0-678")))
	g.Expect(configStore.DeletedIstioConfigs).Should(ContainElement(Equal("gateway:istio-egressgateway-svc-0-678")))
	g.Expect(configStore.DeletedIstioConfigs).Should(ContainElement(Equal("virtual-service:egress-gateway-svc-0-678")))
	g.Expect(configStore.DeletedIstioConfigs).Should(ContainElement(Equal("virtual-service:mesh-to-egress-svc-0-678")))
	g.Expect(configStore.DeletedIstioConfigs).Should(ContainElement(Equal("service-entry:svc-0-678-service")))
	g.Expect(configStore.DeletedIstioConfigs).Should(ContainElement(Equal("destination-rule:sidecar-to-egress-svc-1-678")))
	g.Expect(configStore.DeletedIstioConfigs).Should(ContainElement(Equal("destination-rule:egressgateway-svc-1-678")))
	g.Expect(configStore.DeletedIstioConfigs).Should(ContainElement(Equal("gateway:istio-egressgateway-svc-1-678")))
	g.Expect(configStore.DeletedIstioConfigs).Should(ContainElement(Equal("virtual-service:egress-gateway-svc-1-678")))
	g.Expect(configStore.DeletedIstioConfigs).Should(ContainElement(Equal("virtual-service:mesh-to-egress-svc-1-678")))
