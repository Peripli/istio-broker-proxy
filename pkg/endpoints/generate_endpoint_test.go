...
func parseResponseData(data []byte, t *testing.T) responseData {
	...
	if err != nil {
		t.Fatalf("error while decoding body: %s (%v)", err.Error(), data)
	}
	return result
}
...
func TestEndpointIsCreated(t *testing.T) {
	...
	body := []byte(`{
            "credentials": {
               "uri": "postgres://mysqluser:pass@mysqlhost:3306/dbname",
               "hostname": "mysqlhost",
               "port": "3306"
			} }`)
	...
	if err != nil {
		t.Fatalf("error while generating endpoints: %s", err.Error())
	}
...
func TestEndpointDataIsCorrect(t *testing.T) {
	...
	g.Expect(actualHost).To(Equal("mysqlhost"))
}
